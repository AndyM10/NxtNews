...
export const getUser = async (userToken: string): Promise<NxtUser | undefined> => {
  ...
  try {
    ...
    if (!userAuthData) throw ('Id Token failed to verify')

    const user = await firebaseAdmin.firestore().doc(`users/${userAuthData.uid}`).get()
    if (!user) throw ('Failed to fetch user doc')
